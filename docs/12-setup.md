...
🌐 [English](en/12-setup.md) · **Русский**

Установка — не цель, а подготовка. После неё ты откроешь редактор и попробуешь первую сессию pair programming.

Выбери свой маршрут:

- **Минимальный setup** — один редактор, один проект, один стек.
- **Рабочий setup для тимлида** — несколько репозиториев, SSH, CI, общие правила для команды.

---

## Маршрут А. Минимальный setup для быстрого старта

Подойдёт, если хочешь просто попробовать. Никакой инфраструктуры, один проект, один редактор.
...
- **Где скачать:** https://www.kimi.com/code
...
## Маршрут Б. Рабочий setup для тимлида

Когда работаешь с несколькими репозиториями и командой, минимального setup мало. Нужны правила, инфраструктура и единый контекст.

### Дополнительно понадобится

1. SSH-ключи для серверов и GitHub.
2. CI/CD: GitHub Actions, GitLab CI или аналог.
3. Единый `AGENTS.md` или шаблон для команды.
4. Workspace-контекст, если проектов несколько.

### SSH

SSH нужен, чтобы заходить на сервер (VDS/VPS) и работать с GitHub по ключу.
...
Скопируй вывод и добавь в **Settings → SSH and GPG keys**.
...
`workspace-context.md` — это карта связей: какие контракты между проектами, где источник истины, куда коммитить изменения при cross-repo задачах.
...
## Проекты
- project-a: Python FastAPI, источник API-контрактов.
- project-b: Next.js frontend, потребитель API.

## Контракты
- /api/v1/orders: project-a → project-b.
- События order.created: project-a → RabbitMQ → project-b.

## Правила
- API-контракты меняем только через ADR.
- Перед правкой в project-a проверяем потребителей в project-b.
```

### CI/CD

Настрой автопроверки для каждого репозитория. Пример GitHub Actions:
...
Агент должен знать, какие проверки запускаются в CI. Укажи это в `AGENTS.md`:
...
### Шаблон AGENTS.md для команды

Создай единый шаблон и положи в `templates/AGENTS.md`. При старте нового проекта копируй его в корень и адаптируй. Пример шаблона — в [`templates/AGENTS.md`](../templates/AGENTS.md).

### Рабочий чеклист тимлида

- [ ] SSH-ключи настроены и работают.
- [ ] Репозитории организованы в workspace.
- [ ] В каждом проекте есть `AGENTS.md`.
- [ ] CI запускает тесты и линтер.
- [ ] Есть шаблон сессионной карточки — [`templates/session-template.md`](../templates/session-template.md).
- [ ] Команда знает: агент не пушит, diff читает человек.

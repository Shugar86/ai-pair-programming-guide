...
🌐 [English](en/13-git-github.md) · **Русский**
...
Агент хорошо пишет код, плохо несёт ответственность за историю. Git — твоя зона.

### Не давай агенту пушить

Push — это публикация. Только ты решаешь, когда изменения готовы уйти в общий репозиторий. Агент может подготовить diff, написать сообщение коммита, собрать PR — но не нажимает `git push`.

**Почему:** агент не чувствует контекста команды, не знает, что сейчас в `main`, и не понимает, стоит ли вливать изменения прямо сейчас.

**Как сделать:**
- В CLI не давай команд типа «запушь» или «отправь на GitHub».
- В IDE применяй diff сам, коммить сам, пушь сам.
- Если агент предлагает push — остановись и переспроси.

### Всегда читай diff перед коммитом

Diff — это ответ на вопрос «что реально изменилось?». Читай его перед каждым коммитом.

```bash
git diff                    # что изменилось, но ещё не добавлено
git diff --staged          # что уже в индексе
```

Что искать:
- лишние файлы — `console.log`, временные файлы, `.env`;
- изменения, которые не относятся к задаче;
- странные правки в соседних файлах;
- захардкоженные значения или закомментированный код.

Чеклист приёмки diff — в [`09-checklists.md`](09-checklists.md).

### Используй агента для сообщений коммитов и описаний PR

Агент отлично формулирует, что изменилось, если дать ему diff.

**Сообщение коммита:**

```text
Вот diff. Напиши сообщение коммита в формате <type>: <что изменилось>.
...
```text
Вот список коммитов и diff. Напиши описание PR:
- что изменилось,
- зачем,
- что проверялось,
...
Проверяй результат. Агент иногда преувеличивает или забывает важную деталь. Сообщение коммита — твоё лицо в истории.
...
- **Не пускай агента push'ить.** Push — твоё решение, не его.

# Контракт проекта для AI-агента

Копируй этот файл в корень своего проекта как `AGENTS.md` и заполняй разделы под себя.

## Общее

- **Язык:** русский / английский
- **Роль агента:** pair-programmer
- **Человек ведёт:** приоритеты, архитектуру, компромиссы
- **Агент помогает:** писать код, проверять, сужать неопределённость
- **Версия стека:** например, Python 3.11, Next.js 14, PostgreSQL 15

## Стек

- **Язык:**
- **Фреймворк:**
- **База данных:**
- **Тесты:**
- **Линтер/форматтер:**

## Правила

- KISS — простейшее решение
- Минимальный diff — трогай только нужное
- YAGNI — без кода "на будущее"
- План до кода — согласуй подход перед реализацией
- Проверяй diff перед применением
- Сомневаешься — спроси

## Git

- Коммить результат задачи.
- Сообщение коммита: `<type>: <что изменилось>`.
- Не коммить `.env`, ключи, локальные настройки.

## Запрещено

- Хардкодить секреты.
- Делать force push.
- Править прод без согласования.
- Притворяться, что всё понятно, если непонятно.
- Делать изменения в более чем 3 файлах без согласования.

## Контакты / эскалация

- Кого спрашивать, если не уверен:
- Как деплоить:
- Где логи:

Пример: если агент предлагает изменить архитектуру — стоп, обсуди с тимлидом.
